...
class SamplesMIXIN(object):
    ...
    def setup(self, str_or_dict, **kwargs):
        ...
        # MCMC chains passed in unsupported format
            ...
            self.logger.error(
                "first argument to samples2getdist should be a file name string, list, tuple or dict."
            )
            raise
    ...
    def chains2samples(self, **kwargs):
        ...
        self.ichain = np.concatenate(
            [(i + 1) * np.ones(len(c)) for i, c in enumerate(self.chains)]
        )
        #
        # before concatnating do burn-in
        if burnlen > 0:
            self.logger.debug(
                "Chain2Sample: applying burn-in with burn length=%s" % burnlen
            )
            self.chains = [self.removeBurn(burnlen, chain=c) for c in self.chains]

        # keep chain index offsets
        self.chain_offsets = np.cumsum(
            np.array([0] + [chain.shape[0] for chain in self.chains])
        )
        ...
        if np.abs(thinlen) > 0:
            self.logger.debug(
                "Chain2Sample: applying weighted thinning with thin length=%s" % thinlen
            )
    ...
    def chain_split(self, s):
        if self.split:
            ...
            s1_dict = {
                "weights": s1[:, self.iw],
                "loglikes": s1[:, self.ilike],
                "samples": s1[:, self.itheta :],
                "ichain": ix,
            }
            s2_dict = {
                "weights": s2[:, self.iw],
                "loglikes": s2[:, self.ilike],
                "samples": s2[:, self.itheta :],
                "ichain": not_ix,
            }
        else:
            ...
            s1_dict = {
                "weights": s[:, self.iw],
                "loglikes": s[:, self.ilike],
                "samples": s[:, self.itheta :],
                "ichain": range(len(s)),
            }
    ...
    def removeBurn(self, remove, chain=None, name=None):
        ...
        if not chain is None:
            try:
                ...
                self.logger.info(
                    "burn-in failed: burn length %s > sample length %s"
                    % (nstart, nsamples)
                )
                raise
    ...
    def poisson_thin(self, thin_retain_frac, name="s1", weights=None):
        ...
        if self.debug:
            print(
                "Poisson thinned chain:",
                len(thin_ix),
                "<w>",
                "{:5.2f}".format(np.mean(weights)),
...
class MCEvidence(object):
    def __init__(
        ...
        **gdkwargs
        ...
        if isfunc:
            ...
            if self.split:
                self.gd.importance_sample(isfunc, name="s2")
            # except:
        ...
        #
        self.logger.info(
            "chain array dimensions: %s x %s =" % (self.nsample, self.ndim)
        )
        #
        self.set_batch()
        ...
        if self.brange is None:
            ...
            if bscale == "logpower":
                ...
                for ix, nn in enumerate(self.nsample):
                    ...
                    self.bsize[:, ix] = np.array(
                        [int(pow(10.0, x)) for x in self.powers]
                    )
                ...
                for ix, nn in enumerate(self.nsample):
                    self.nchain[:, ix] = np.array(
                        [x for x in self.bsize[:, ix].cumsum()]
                    )
    ...
    def get_samples(self, nsamples, istart=0, rand=False, name="s1", prewhiten=True):
        ...
        if rand and not self.brange is None:
            if nsamples > ntot:
                self.logger.error(
                    "partition %s nsamples=%s, ntotal_chian=%s" % (name, nsamples, ntot)
                )
                raise
        ...
        self.logger.info(
            "getting samples for partition %s: nsamples=%s" % (name, nsamples)
        )
        ...
        if prewhiten:
            ...
            try:
                ...
                self.logger.error(
                    "Unknown error during diagonalizing the chain with its covariance matrix."
                )
                raise
        else:
            # no diagonalisation
    ...
    def evidence(
        ...
        for ipow, nsample in zip(self.idbatch, self.nchain):
            ...
            samples, logL, weight, jvv = self.get_samples(
                S, istart=itot, rand=rand, prewhiten=False, name="s1"
            )
            if covtype == "single":
            ...
            if self.split:
                ...
                samples2, logL2, weight2, jvv2 = self.get_samples(
                    0, istart=itot, rand=rand, prewhiten=False, name="s2"
                )
                ...
                samples2 = self.diagonalise_chain(
                    samples2, covstat["eVec"], covstat["eVal"]
                )
            ...
            for k in range(k0, self.kmax):
                for j in range(0, S):
                    ...
                    volume[j, k] = (
                        math.pow(math.pi, ndim / 2)
                        * math.pow(DkNN[j, k], ndim)
                        / sp.gamma(1 + ndim / 2)
                    )
                ...
                #
                txt = (
                    "********SumW={:0.2f},amax={:0.2f},Jacobian={:0.2f},logLmax={:0.2f}"
                )
                self.logger.debug(txt.format(SumW, amax, Jacobian, logLmax))
                #
                MLE[ipow, k] = (
                    math.log(SumW * amax * Jacobian) + logLmax - logPriorVolume
                )

                self.logger.debug("SumW={} \t S={} ".format(SumW, S))
                self.logger.debug("amax={} \t Jacobian={}".format(amax, Jacobian))
                self.logger.debug(
                    "logLmax={} \t logPriorVolume={}".format(logLmax, logPriorVolume)
                )
                ...
                if self.brange is None:
                    ...
                    if verbose > 1:
                        self.logger.debug(
                            "k={},nsample={}, dotp={}, median_volume={}, a_max={}, MLE={}".format(
                                k,
                                S,
                                dotp,
                                statistics.median(volume[:, k]),
                                amax,
                                MLE[ipow, k],
                            )
                        )
                    ...
                    if verbose > 1:
                        if ipow == 0:
                            self.logger.debug(
                                "(iter,mean,min,max) of LogLikelihood: ",
                                ipow,
                                fs.mean(),
                                fs.min(),
                                fs.max(),
...
def params_info(fname, cosmo=False, volumes={}):
    ...
    # MontePython
                    ...
                    if line.find("data.parameters") != -1:
                        ...
                        if pcond and not array[5] == "derived":
                            ...
                            if array[1] == "None" or array[2] == "None":
                                raise Exception(
                                    "Unbounded priors are not supported - please specify priors"
                                )
                            vmin = float(array[1])
                            vmax = float(array[2])
        ...
        raise Exception(
            "Could not read parameter volume from COSMOMC .ranges file or montepython log.param file"
        )
    #
...
def get_prior_volume(args, **kwargs):
    ...
    try:
        ...
        logger.info(
            "getting prior volume using cosmomc *.ranges or montepython log.param outputs"
        )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        prog=sys.argv[0], add_help=True, description=desc, epilog=cite
    )

    # positional args
    parser.add_argument(
        "root_name", help="Root filename for MCMC chains or python class filename"
    )

    vstring = ">>>   %(prog)s :: {0} version date: {1}   <<<"
    parser.add_argument(
        "--version", action="version", version=vstring.format(__status__, __version__)
    )
    # optional args
    parser.add_argument(
        "-k", "--kmax", dest="kmax", default=2, type=int, help="scikit maximum K-NN "
    )
    parser.add_argument(
        "-ic",
        "--idchain",
    ...
    parser.add_argument(
        "--allparams",
        help="flag to use all params and not use iscosmo_params condition",
        action="store_true",
    )
    ...
    if args.paramsfile != "":
        ...
        print(
            "Adding additional parameter names to cosmo_params list from %s.."
            % args.paramsfile
        )
    ...
    print(
        "* ln(B)[k] is the natural logarithm of the Baysian evidence estimated using the kth Nearest Neighbour."
    )
    print("")
